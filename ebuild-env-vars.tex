...
\item[CHOST,CBUILD,CTARGET] If not set by profiles, must contain either an appropriate
    ...
    be unset.
\item[EBUILD\_PHASE] Takes one of the values \t{config}, \t{setup}, \t{nofetch}, \t{unpack},
    \t{compile}, \t{test}, \t{install}, \t{preinst}, \t{postinst}, \t{prerm}, \t{postrm}, \t{info}
    according to the top level ebuild function that was executed by the package manager.
